      ...
      <div>
        <div>
          <h2 class="no-background">
            <span>{{userInfo.userNickName}}의 타임라인</span>
          </h2>
        </div>
...
export default {
  ...
  methods: {
    ...
    followInfo (id) {
      this.$http.get(
        this.$store.state.base_url + 'accounts/follows/' + id
      ).then(res => {
    ...
    followerInfo (id) {
      this.$http.get(
        this.$store.state.baseUrl + 'accounts/followers/' + id
      ).then(res => {
    ...
    addFollow () {
      this.$http.post(
        this.$store.state.baseUrl + 'accounts/following/' +
        this.$store.state.user_id + '/' + this.userInfo.id
      ).then(res => {
        if (this.followState) {
    ...
    getUserInfo (id) {
      ...
      form.append('id', id)
      this.$http.post(
        this.$store.state.baseUrl + 'accounts/userInfo', form
      ).then(res => {
        this.userInfo = res.data
        if (res.data.userLike !== '') {
          let strArray = res.data.userLike.split(',')
          this.tags = strArray
        }
        let image = res.data.userImage
        if (image.split('/').slice(3).join('/')) {
          this.userInfo.userImage = 'https://' +
      ...
      let counts = 0
      this.$http.get(
        this.$store.state.baseUrl + 'boards/participantCheckListByUser/' + id
      ).then(res => {
        let band = res.data
